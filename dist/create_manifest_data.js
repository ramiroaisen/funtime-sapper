...
const {encode, decode} = sourcemapCodec__default['default'];

function hash(content) {
	return crypto__default['default'].createHmac('sha256', content)
		.digest('hex')
		.substr(0, 8);
}
			...
			for (const chunk of Object.values(bundle).reverse()) {
				...
				for (const f of Object.keys(chunk.modules).filter(filter)) {
					if (data.map[f]) {
						...
						sources.push(path__default['default'].relative(options.dir, data.map[f].sources[0]));
